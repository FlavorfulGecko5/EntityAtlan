...
int gcm_start( gcm_context *ctx,    // pointer to user-provided GCM context
               ...
               unsigned int iv_len,       // IV length in bytes (should == 12)
               const uchar *add,    // ptr to additional AEAD data (NULL if none)
               unsigned int add_len )     // length of additional AEAD data (bytes)
{
    ...
    unsigned int use_len;     // byte count to process, up to 16 bytes
    unsigned int i;           // local loop iterator
...
int gcm_update( gcm_context *ctx,       // pointer to user-provided GCM context
                unsigned int length,          // length, in bytes, of data to process
...
int gcm_finish( gcm_context *ctx,   // pointer to user-provided GCM context
                ...
                unsigned int tag_len )    // length, in bytes, of the tag-receiving buf
{
    uchar work_buf[16];
...
int gcm_crypt_and_tag(
        ...
        unsigned int iv_len,          // byte length if the IV. should always be 12
        const uchar *add,       // pointer to the non-ciphered additional data
        unsigned int add_len,         // byte length of the additional AEAD data
        const uchar *input,     // pointer to the cipher data source
        uchar *output,          // pointer to the cipher data destination
        unsigned int length,          // byte length of the cipher data
        uchar *tag,             // pointer to the tag to be generated
        unsigned int tag_len )        // byte length of the tag to be generated
{   /*
...
int gcm_auth_decrypt(
        ...
        unsigned int tag_len )        // byte length of the tag <= 16
{
    uchar check_tag[16];        // the tag generated and returned by decryption
    int diff;                   // an ORed flag to detect authentication errors
    unsigned int i;                   // our local iterator
    /*
